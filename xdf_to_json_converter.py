...
class XdfToJsonConverter:
    ...
    def convert_to_json(self):
        ...
        print(f"Loaded data sucessfully")
    ...
    def extract_pupil_size_list(self):
        for stream in self.json_data.get('streams', []):
            ...
            if 'pupil' in stream_name.lower():
                ...
                if not isinstance(ts, list) or (ts and not isinstance(ts[0], list)):
                    return ts
...
converter = XdfToJsonConverter('./sub-P008_ses-S001_task-Default_run-001_eeg.xdf')

pupil = converter.get_pupil_data()
